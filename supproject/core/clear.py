...
def get_test(p_id, uid, user_book_id):
    ...
    sql = """SELECT id,level_status,s_json_data,clear_time FROM {mark}test WHERE user_id={uid} AND user_book_id={cid}""".format(
        uid=uid, cid=user_book_id,mark=mark)
...
def get_level_sql(tid, lid,mark, iscurrent=True):
    """清楚模板  数理化预科的数据结构"""

    sql_1 = """UPDATE {mark}knowledge_study SET id=id+{n},test_id=test_id+{n} WHERE test_id={id};""".format(n=n, id=tid,mark=mark)
    sql_2 = """UPDATE {mark}method_study SET id=id+{n},test_id=test_id+{n} WHERE test_id={id};""".format(n=n, id=tid,mark=mark)
    sql_3 = """UPDATE {mark}example_study SET id=id+{n},test_id=test_id+{n} WHERE test_id={id};""".format(n=n, id=tid,mark=mark)
    sql_4 = """UPDATE {mark}apply_test SET id=id+{n},test_id=test_id+{n} WHERE test_id={id} AND level_id={lid};""".format(
        n=n, id=tid, lid=lid,mark=mark
    )
    if iscurrent:
...
def clear_level(p_id, uid, cid, lid,user_book_id):
    ...
    try:
        ...
        sql_1 = """UPDATE {mark}test SET level_id={lid},level_status={status},position=0,
            ...
            lid=l_id, status=level_status, now=update_time, id=id, clear_time=clear_time, data=str(json.dumps(data)),mark=mark
        )
...
def clear_c_l(p_id, uid, c_id, lid,user_book_id):
    ...
    try:
        ...
        conn_db(db_name, sql_1, 57)
        sql_2 = get_level_sql(id, lid,mark, iscurrent=False)
        conn_db(db_name, sql_2, 57, many=True)
        time1 = time.strftime('%m/%d %H:%M:%S', time.localtime(update_time))
...
def clear_all(p_id, uid, c_id, l_id,user_book_id):
    ...
    db_name,mark = get_db_name(p_id)
    # ************清除所有关卡数据
    sql1 = """select id from {mark}test where user_id={user_id} and user_book_id={user_book_id}""".format(mark=mark,user_id=uid,user_book_id=user_book_id)
